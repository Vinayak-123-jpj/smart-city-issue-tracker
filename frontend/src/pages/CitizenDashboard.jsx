...
import { IssueCardSkeleton, StatCardSkeleton } from '../components/common/Skeleton';
    ...
    try {
      ...
      setIssues(prevIssues => 
        prevIssues.map(issue => 
          issue._id === issueId 
            ? { ...issue, upvoteCount: response.data.upvoteCount, upvotedBy: response.data.upvotedBy }
            : issue
        )
      );
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 animate-slide-down">
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-3xl font-bold text-gray-900">Citizen Dashboard</h2>
              <p className="mt-1 text-gray-600">Report and track civic issues</p>
            </div>
            <button 
              onClick={() => setShowCreateForm(true)} 
              className="btn-primary flex items-center space-x-2 hover-lift btn-press"
            >
        ...
        {!loading && !error && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 hover-lift card-enter" style={{ animationDelay: '0.1s' }}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-600">Pending</p>
                  <p className="text-2xl font-bold text-yellow-600">{issues.filter((i) => i.status === 'Pending').length}</p>
                </div>
            ...
            <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 hover-lift card-enter" style={{ animationDelay: '0.2s' }}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-600">In Progress</p>
                  <p className="text-2xl font-bold text-blue-600">{issues.filter((i) => i.status === 'In Progress').length}</p>
                </div>
            ...
            <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 hover-lift card-enter" style={{ animationDelay: '0.3s' }}>
              <div className="flex items-center justify-between">
                <div>
        ...
        {!loading && !error && issues.length > 0 && (
          <IssueFilters filters={filters} onFilterChange={handleFilterChange} issueCount={filteredIssues.length} />
        )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4">
              ...
              <p className="text-sm text-gray-600 mt-1">Help improve our city</p>
            </div>
...
/>
      ...
      {/* Issue Detail Modal */}
      {selectedIssue && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-fade-in" onClick={() => setSelectedIssue(null)}>
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-start">
              <div>
                ...
                <div className="flex items-center space-x-3 mt-2">
                  ...
                  <span className="text-sm text-gray-600">👍 {selectedIssue.upvoteCount || 0} upvotes</span>
                </div>
              </div>
              <button 
            ...
            <div className="px-6 py-6 space-y-6">
              ...
              <div className="pt-4 border-t border-gray-200">
                <div className="flex items-center justify-between text-sm text-gray-500">
                  <span>Reported {new Date(selectedIssue.createdAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
